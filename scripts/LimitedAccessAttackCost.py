...
class LimitedAccessAttackCost:
    
    def __init__(self, house_name, dataframe, cleaned_dataframe_occ_1, cleaned_dataframe_occ_2, processed_dataframe, list_time_min_occ_1, list_time_max_occ_1, list_time_min_occ_2, list_time_max_occ_2, attack_schedule_occ_1, attack_schedule_occ_2, dict_control_cost, num_timeslots, num_zones, activity_zone_map, zone_cost, att_start_time, att_end_time, accessible_zones):
        ...
        self.att_start_time         = att_start_time
        self.att_end_time           = att_end_time
        self.accessible_zones       = accessible_zones
    ...
    def get_attack_costs_without_appliance_triggering(self):
        ...
        for j in range(0, len(self.dataframe), self.num_timeslots):
            ...
            for i in range(len(self.attack_schedule_occ_1)):
                ...
                if i >= self.att_start_time and i <= self.att_end_time:
                    
                    benign_occ_1_zone = self.activity_zone_map[self.processed_dataframe['Occupant 1 Activity'][i]]
                    benign_occ_2_zone = self.activity_zone_map[self.processed_dataframe['Occupant 2 Activity'][i]]
                    
                    if control_zone_occupants[0] == 2:
                        zone_occupants[0] = 2
                    elif control_zone_occupants[0] == 1:
                        zone_occupants[0] = 1
                        if benign_occ_1_zone in self.accessible_zones and int(self.attack_schedule_occ_1[i]) in self.accessible_zones:
                            zone_occupants[int(self.attack_schedule_occ_1[i])] += 1
                        else:
                            zone_occupants[benign_occ_1_zone] += 1
                        ...
                        if benign_occ_2_zone in self.accessible_zones and int(self.attack_schedule_occ_2[i]) in self.accessible_zones:
                            zone_occupants[int(self.attack_schedule_occ_2[i])] += 1
                        else:
                            zone_occupants[benign_occ_2_zone] += 1
                        
                else:
                    zone_occupants = control_zone_occupants.copy()
    ...
    def get_attack_costs_with_appliance_triggering(self):
        ...
        for i in range(0, 43200, 1440):
            ...
            for j in range(len(attackable)):
                if attackable[j] == 1 and j >= self.att_start_time and j <= self.att_end_time:
                    COST_PER_KWH = 0.395
